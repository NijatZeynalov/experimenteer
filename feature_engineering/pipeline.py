...
missing_dict = {'mean_imputer': ['from feature_engine.imputation import MeanMedianImputer',
...
X_test = imputer.transform(X_test)"""
                ...
                'arbitrary_value_imputer': ['from feature_engine.imputation import ArbitraryNumberImputer',
 ...
 X_test = imputer.transform(X_test)"""],
                ...
                'missing_indicator_imputer': ['from feature_engine.imputation import CategoricalImputer, AddMissingIndicator',
...
X_test = imputer.transform(X_test)"""],
                ...
                'missing_category_imputer': ['from feature_engine.imputation import CategoricalImputer',
...
X_test = imputer.transform(X_test)"""],
                ...
                'iterative_imputer': ["""from sklearn.linear_model import BayesianRidge
...
encode_dict = {'one_hot_encoder': ['from feature_engine.encoding import OneHotEncoder',
  ...
  X_test = encoder.transform(X_test)"""
               ...
               'ordinal_encoder': ['from feature_engine.encoding import OrdinalEncoder',
                                   """encoder = OrdinalEncoder(encoding_method="arbitrary")
encoder.fit(X_train)
X_train= encoder.transform(X_train)
X_test = encoder.transform(X_test)"""],

               'frequency_encoder': ['from feature_engine.encoding import CountFrequencyEncoder',
                                     """encoder = CountFrequencyEncoder(encoding_method="frequency")
  encoder.fit(X_train)
  X_train= encoder.transform(X_train)
  X_test = encoder.transform(X_test)"""],

               'mean_encoder': ['from feature_engine.encoding import MeanEncoder',
                                """encoder = MeanEncoder()
encoder.fit(X_train)
X_train= encoder.transform(X_train)
X_test = encoder.transform(X_test)"""],

               'woe_encoder': ['from feature_engine.encoding import WoEEncoder',
                               """encoder  = WoEEncoder()
encoder.fit(X_train)
X_train= encoder.transform(X_train)
X_test = encoder.transform(X_test)"""],

               'ohe_frequent_encoder': ["""from feature_engine.encoding import OneHotEncoder
""",
...
outlier_dict = {'removing_outliers_iqr': ['from feature_engine.outliers import OutlierTrimmer', """trimmer = OutlierTrimmer(
capping_method="iqr",
tail="both",
fold=1.5,
)
...
y_test = y_test[y_test.index.isin(X_test.index)]"""],
                'none': ['',''],
                'removing_outliers_quantiles': ['from feature_engine.outliers import OutlierTrimmer', """trimmer = OutlierTrimmer(
...
feature_selection_dict = {'select_k_best':['from sklearn.feature_selection import SelectKBest, RFE',
...
"""],
                         'recursive_feature_elimination':['from sklearn.feature_selection import SelectKBest, RFE',
...
scaling_dict = {'standard':['from sklearn.preprocessing import StandardScaler',"""
...
"""],
          'min_max':['from sklearn.preprocessing import MinMaxScaler',"""
...
def notebook_generator(dataset, target, missing, encode, outlier, selection, scaling, model):
        ...
        nb['cells'] = [nbf.v4.new_markdown_cell(text_1),
...
class CustomPipeline:
    ...
    def method_transform(self):
        ...
        for step in self.steps:
            X_train_t, X_test_t, y_train, y_test = step.method_transform(X_train_t, X_test_t, y_train, y_test)
...
class DatasetCleaner:
    ...
    def clean(self):
        """Cleans the dataset by dropping duplicates, collinear features, unique features, and other features as specified."""
